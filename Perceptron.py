...
def step_function(x):
    threshold = 0.2
    return np.heaviside(np.subtract(x, threshold), 0.5)
...
plt2 = plt.subplot2grid((2,3), (1,0), colspan=3)
train, = plt2.plot(train_mse, label = 'Train')
test, = plt2.plot(test_mse, label = 'Test')
plt2.set_title("MSE Over Time")
plt2.set_xlabel("Epochs")
plt2.set_ylabel("Mean Squared Error")
plt2.legend([test, train], ['Test', 'Train'])
